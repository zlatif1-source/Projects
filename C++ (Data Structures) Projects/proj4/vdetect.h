...
class VDetect{
 ...
 private:
  ...
  int probeIndex(string key, int i, int cap, prob_t policy) const;
  int findIndex(Virus** table, int cap, prob_t policy, string key, int id) const;
  bool insertIntoTable(Virus** table, int cap, prob_t policy,
		       Virus virus, int& tableSize, int& numDeleted);
  void startRehash();
  void transferOldTable();
  void clearTable(Virus** table, int cap);
  int nextPrimeGap(int current);
